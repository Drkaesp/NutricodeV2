...
import { fetchFoodsFromApi } from '@/src/utils/api';
...
export default function AlimentacaoEdit() {
  ...
  async function loadCurrentFoods() {
    ...
    const apiData = await fetchFoodsFromApi();
    setApiFoods(apiData);
    setLoading(false);
  }
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.header}>
        <TouchableOpacity onPress={() => router.replace('/(panel)/alimentacao/page' as any)} style={styles.backBtn}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {loading && search.length > 1 ? (
          <Text style={{textAlign: 'center', color: Colors.textMuted, marginVertical: 20}}>Carregando da API...</Text>
